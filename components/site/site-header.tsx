  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? 'bg-ink/85 backdrop-blur-md py-3 shadow-lg shadow-black/20' : 'bg-transparent py-5'}`}>
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 md:px-8">
        <a href="/#top" className="flex items-center" aria-label="Visões do Rio — início"><img src="/logos/horizontal-1.png" alt="Visões do Rio" className="h-9 w-auto md:h-10" /></a>
        ...
        <div className="flex items-center gap-3"><a href="/contato" className="hidden rounded-full border border-yellow px-6 py-2 text-sm font-semibold text-yellow transition-colors hover:bg-yellow hover:text-ink sm:inline-block">Contato</a><button type="button" aria-label="Abrir menu" aria-expanded={open} onClick={() => setOpen((v) => !v)} className="flex h-10 w-10 flex-col items-center justify-center gap-1.5 lg:hidden"><span className={`h-0.5 w-6 bg-cream transition-transform ${open ? 'translate-y-2 rotate-45' : ''}`} /><span className={`h-0.5 w-6 bg-cream transition-opacity ${open ? 'opacity-0' : ''}`} /><span className={`h-0.5 w-6 bg-cream transition-transform ${open ? '-translate-y-2 -rotate-45' : ''}`} /></button></div>
      </div>
